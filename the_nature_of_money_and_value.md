...
```mermaid
graph TD
    A[Theories of Money] --> B[1. Commodity Theory / Metallism]
    A --> C[2. State Theory / Chartalism]
    A --> D[3. Credit & Debt Theory]
    
    B --> B1["Money originates as a highly marketable good<br>(Menger, Adam Smith, Aristotle)"]
    C --> C1["Money is a creature of the state & law<br>(Knapp, Keynes, MMT)"]
    D --> D1["Money is a unit of social debt & ledger<br>(Innes, Graeber, Martin)"]
```
...
```mermaid
graph LR
    A[Digital Money Frontiers] --> B[Cryptocurrencies<br>Bitcoin, Ethereum]
    A --> C[Stablecoins<br>USDT, USDC]
    A --> D[Central Bank Digital Currencies<br>CBDCs]
    
    B --> B1["Mechanizes 'Money is Memory'<br>via Trustless Cryptographic Proof"]
    C --> C1["Tokenized Private Bank/Reserve Claims"]
    D --> D1["Direct Digital Liability of the Sovereign"]
```
